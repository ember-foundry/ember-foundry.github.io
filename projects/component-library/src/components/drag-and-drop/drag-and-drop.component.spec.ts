...
describe('DragAndDropComponent', () => {
  let mock_drag_event: MockDragEvent;
  let mock_file_selection_event: MockFileSelectionEvent;
  ...
  describe('drag events', () => {
    ...
    it('on_drag_start should set effectAllowed', () => {
      component['on_drag_start'](mock_drag_event as unknown as DragEvent);
      expect(mock_drag_event.stopPropagation).toHaveBeenCalled();
      expect(mock_drag_event.preventDefault).toHaveBeenCalled();
    ...
    it('on_drag_over should prevent default', () => {
      component['on_drag_over'](mock_drag_event as unknown as DragEvent);
      expect(mock_drag_event.stopPropagation).toHaveBeenCalled();
      expect(mock_drag_event.preventDefault).toHaveBeenCalled();
    });
    ...
    it('on_drag_leave should reset', () => {
      ...
      component['on_drag_leave'](mock_drag_event as unknown as DragEvent);
      expect(mock_drag_event.stopPropagation).toHaveBeenCalled();
      expect(mock_drag_event.preventDefault).toHaveBeenCalled();
  ...
  describe('on_drop', () => {
    it('should handle valid files drop', () => {
      ...
      mock_drag_event.dataTransfer!.files = [file];

      const spy = vi.spyOn(component.files_requested, 'emit');

      component['on_drop'](mock_drag_event as unknown as DragEvent);
    ...
    it('should reset if no files in drop', () => {
      mock_drag_event.dataTransfer!.files = [];
      component['dragging'].set(true);
    ...
    it('should handle missing dataTransfer in drop', () => {
      mock_drag_event.dataTransfer = undefined;
      component['dragging'].set(true);
      component['on_drop'](mock_drag_event as unknown as DragEvent);
      expect(component['dragging']()).toBe(false);
    });
  ...
  describe('on_file_selected', () => {
    ...
    it('should handle valid files from input', () => {
      ...
      const spy = vi.spyOn(component.files_requested, 'emit');
      component['on_file_selected'](mock_file_selection_event as unknown as Event);
      expect(spy).toHaveBeenCalledWith([file]);
      expect(component['valid']()).toBe(true);
      expect(component['file_count']()).toBe(1);
    });
