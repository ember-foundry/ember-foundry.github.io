...
import {ChipCritterComponent} from 'component-library';

const meta: Meta<ChipCritterComponent> = {
  component: ChipCritterComponent,
  tags: ['autodocs', 'critter', 'chip'],
  argTypes: {
    color: {
...
export const Sample: StoryObj<ChipCritterComponent> = {}
  ...
  render: () => ({
    ...
    template: `
        @for(color of colors; track color) {
            <mbr-chip-critter [color]="color">{{color}}</mbr-chip-critter>
        }
        ...
        <mbr-chip-critter color="success-tonal">
          <ng-container [ngTemplateOutlet]="icon_check" />
          STOCKED
        </mbr-chip-critter>

        <mbr-chip-critter color="warning-tonal">
